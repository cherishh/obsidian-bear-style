...
![Bear Style 浅色海报：真实 Obsidian 正文、列表、引用与表格。](cover.png)
...
这是一个**独立的 Obsidian 主题**，同时保留默认主题使用的 CSS 代码片段和可选桌面端光标插件。基础主题无需社区插件；主题与代码片段二选一即可。浅色模式采用 Bear 的 **Red Graphite** 配色，暗色模式采用 **Dark Graphite** 的蓝色强调色与炭灰背景。
...
两张海报使用 **macOS、Obsidian 1.13.7** 的真实浅色与暗色截图，配合宣传文字排版；文档的字体、颜色与图标均直接来自 Obsidian。下方长图使用**默认主题、浅色模式**，加上与正式主题共享的 Bear Style CSS。长图展示阅读视图，光标特写展示实时阅览。仓库不附带第三方插件代码、字体或个人配置。
...
在 Obsidian 外观设置中切换浅色或暗色模式。暗色模式将 Bear Dark Graphite 配色应用于正文、标题、链接、高亮、代码、表格与界面背景，同时适配可选的 Code Styler 背景及 Highlightr 高亮类；笔记中直接指定的内联高亮颜色仍由用户控制。

## 兼容性与卸载

- 已在 macOS、Obsidian 1.13.7、默认主题下检查实时阅览和阅读视图中的排版、列表、引用、高亮、图片与代码。
- 已检查正文、标题前、链接后、有序列表旁的光标，以及关闭插件后的原生光标回退。
- 已在 macOS 的实时阅览和阅读视图中对照原生 Bear Dark Graphite 检查暗色效果；海报展示两种模式，长图为浅色模式。Windows、Linux 和实体移动设备尚未测试。
